<template>
    <view >
      ...
      <scroll-view scroll-y scroll-with-animation  class="deviceListWrap">
        ...
        <uni-collapse  @change="onChange" accordion="true">
            <uni-collapse-item :title="item.name" v-for="(item,index) in deviceList" :key="index">
			  ...
			  <!-- //小程序自定义组件slot的使用，在自定义的组件中使用插槽slot，"title"是slot类型，显示{item.name}值，从collapse获取数据 -->
      ...
      <view class="connectedStatus">
		...
		<button size="defult" type="defult" @click="notifyBLECharacteristicValueChange" style="margin: 20upx 0;"> 启用NOTIFY </button>
						<view class="ft-color-red">
							读取NOTIFY返回mac地址: ({{macAddress}})
						</view>
						<view class="ft-color-red">
							读取NOTIFY返回数据: ({{macValue}})
						</view>
